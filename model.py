...
class ZIVGADecoder(GAE):
    def __init__(self, encoder, decoder=None, decoder_nn_dim1=None):
        super(ZIVGADecoder, self).__init__(encoder, decoder)
        ...
        if decoder_nn_dim1:
            ...
            self.x_bar_layer=nn.Linear(in_features=self.decoder_nn_dim1, out_features=self.encoder.in_channels)
            self.dec_mean = nn.Sequential(nn.Linear(self.decoder_nn_dim1, self.encoder.in_channels),MeanAct())
            self.dec_disp = nn.Sequential(nn.Linear(self.decoder_nn_dim1, self.encoder.in_channels),DispAct())
            self.dec_pi = nn.Sequential(nn.Linear(self.decoder_nn_dim1, self.encoder.in_channels),nn.Sigmoid())
            self.zinb_loss = ZINBLoss()

    def forward(self,z):
        decoder_output = self.decoder_nn(z)
        x_bar = self.x_bar_layer(decoder_output)
        mean = self.dec_mean(decoder_output)
        disp = self.dec_disp(decoder_output)
        pi = self.dec_pi(decoder_output)
        return x_bar, mean, disp, pi
